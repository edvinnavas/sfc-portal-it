...
@Named(value = "Cargar_RME_Dialog_Uno")
@ViewScoped
public class Cargar_RME_Dialog_Uno implements Serializable {
    ...
    public void cargar_datos(Usuario usuario, Integer empresa) {
        try {
            ...
            PrimeFaces.current().executeScript("PF('var_dlgSVFiscaliaRMECargar').show();");
        } catch (Exception ex) {
    ...
    public void cargar() {
        ...
        try {
            if (this.documento_transacciones.getFileName() != null && !this.documento_transacciones.getFileName().equals("")) {
                ...
                // path = "C:\\\\rep_svf_fiscalia\\" + fecha_texto + "_RME.xlsx";
                path = "/root/rep_svf_fiscalia/" + fecha_texto + "_RME.xlsx";
            ...
            resultado = servicio.cargarArchivoReporteMensual(Integer.valueOf(this.usuario.getId_usuario().toString()), path, this.empresa);
            if (resultado.trim().substring(0, 5).equals("EXITO")) {
                ...
                // REGISTRAR EVENTO.
                Evento evento = new Evento(Long.valueOf(28), this.usuario.getId_usuario(), Long.valueOf(dataFormat.format(fecha_actual)), "UNO: Cargar transacciones reporte mensual de efectivo.");
