...
@Named(value = "Reporte_Mensual_Efectivo_Uno")
@ViewScoped
public class Reporte_Mensual_Efectivo_Uno implements Serializable {

    private static final long serialVersionUID = 1L;

    private Usuario usuario;

    private List<RME_Uno_List> lista_rme;
    private RME_Uno_List sel_rme;
    private Integer empresa;
    private String titulo;

    public Reporte_Mensual_Efectivo_Uno() {
        this.lista_rme = new ArrayList<>();
    }
    ...
    public void cargar_datos(Usuario usuario, Integer empresa) {
        try {
            ...
            switch (this.empresa) {
                case 1: {
                    this.titulo = "Reporte mensual de efectivo (UNO El Salvador).";
                    break;
                }
                case 2: {
                    this.titulo = "Reporte mensual de efectivo (ECSA El Salvador).";
                    break;
                }
            }
            ...
            for (Integer i = 1; i < filas; i++) {
                RME_Uno_List rme = new RME_Uno_List(
                ...
                this.lista_rme.add(rme);
            }

            PrimeFaces.current().executeScript("PF('var_tblSVFiscaliaRME').clearFilters();");
    ...
    public void eliminar_trans() {
        try {
            if (this.sel_rme != null) {
                if (this.sel_rme.getCodigo_mensaje() != 100) {
                    Svf_Servicio servicio = new Svf_Servicio();
                    String resultado = servicio.eliminarTransaccion(Integer.valueOf(this.usuario.getId_usuario().toString()), this.sel_rme.getId_trans(), 2);
                    if (resultado.trim().substring(0, 5).equals("EXITO")) {
                        ...
                        // REGISTRAR EVENTO.
                        Evento evento = new Evento(Long.valueOf(29), this.usuario.getId_usuario(), Long.valueOf(dataFormat.format(fecha_actual)), "UNO: Eliminar transacciones reporte mensual de efectivo: " + this.sel_rme.getId_trans());
    ...
    public List<RME_Uno_List> getLista_rme() {
        return lista_rme;
    }

    public void setLista_rme(List<RME_Uno_List> lista_rme) {
        this.lista_rme = lista_rme;
    }

    public RME_Uno_List getSel_rme() {
        return sel_rme;
    }

    public void setSel_rme(RME_Uno_List sel_rme) {
        this.sel_rme = sel_rme;
    }
